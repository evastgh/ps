...
\usepackage{fancyhdr}\pagestyle{fancy}\rhead{April 12, 2019}\lhead{Math 33A -- Quiz 2}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
\noindent1. Consider the following,
$$f(x, y) = \l[{2x+y \atop x-y}\r], \qquad g(x, y) = \l[{x+y \atop -y}\r]$$
Define $h(x, y) = g\o f(x, y)$. Find the matrix such that 
$$h(x, y) = A\l[{x \atop y}\r]$$
\\
\noindent2. Let $n = (1, 2, 3)^T \in \R^3$. Let $T$ be the orthogonal projection onto the line $\{tn : t \in \R\}$, and $S$ be the reflection about the plane $\{x : n\cdot x = 0\}$. Are $T$ and $S$ invertible? Why and why not?\\
\textit{Hint:} You can use the fact that an injective and surjective map is invertible. You only need to provide a prose explanation.
