...
\usepackage{fancyhdr}\pagestyle{fancy}\rhead{April 19, 2019}\lhead{Math 33A -- Quiz 3}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
\noindent1. Write down the definition of linear dependence and use it to show that the set 
$$S = \l\{
	\l[
	\begin{array}{c}
		1 \\
...
\\
\noindent2. True or false: \\
(a) Invertible matrices have linearly independent columns. \\
(b) $\mbox{im}(T)$ is spanned by the rows of the matrix $A$, where $T(x) = Ax$. \\
(c) Invertible matrices always induce invertible linear transformations, i.e. if $A \in \R^{n\x n}$ is invertible, then $T(x) = Ax$ defines an invertible map (injective and surjective). \\
(d) $T: \R^2 \ra1 \R^4$ is a linear map. $T$ cannot be injective (one-to-one).
