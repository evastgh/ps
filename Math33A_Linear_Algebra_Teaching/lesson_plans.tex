...
\subsection*{Sec 8.1 Symmetric Matrices}
\begin{itemize}
    \item thm 8.1.1 spectral theorem
    \item eigenspace are linear independent. For symmetric matrices, they are orthogonal
    \item thm 8.1.3
    \item thm 8.1.4 how to find orthogonal diagonalization
\end{itemize}

\subsection*{Sec 7.2 Quadratic Forms}
\begin{itemize}
    \item example 1 (turning quadratic forms into symmetric matrices)
    \item the benefits of diagonalizing the matrix associated with quadratic form (completing the squares)
    \item Definition 8.2.3 (positive/negative definite)
    \item Thm 8.2.4 (link of definiteness with EWs)
    \item Thm 8.2.7 (Ellipse or hyperbola)
\end{itemize}

\subsection*{Sec 8.3 Singular Values}
\begin{itemize}
    \item Review all decomposition we went over in the past: $A = QR, A = SDS^{-1}, A = SDS^T$
    \item Singular value decomposition $A = U\Sigma V^T$
    \item image of the unit circle
    \item Example 4
    \item Example 3
    \item Thm 8.3.4 \#(nonzero singular values) = rank
    \item polar decomposition $A = (UV^T)(V\Sigma V^T) = RS$
    \item matrix norm $\|A\| = \max_{\|x\|=1}\|Ax\|$
\end{itemize}

\subsection*{Final review}
\begin{itemize}
    \item 2 by 2 singular value decomposition
    \item matrix norm
    \item quadratic form, its symmetric matrix, completion of squares, definiteness, maximum and minimum, principal axes
    \item dynamical system, limit behavior
    \item eigenvalue decomposition, links with rank, kernel, similar matrices, invertibility, etc
    \item determinant, geometric meaning, volume of parallelepiped 
    \item QR factorization: GS, computing R, orthogonal projection, lease-squares solutions
    \item orthogonal projections
    \item rank-nullity theorem, finding bases of rank and kernel
    \item conditions of invertibility of a matrix
\end{itemize}
